...
//  UnitTestDemo
//
//  Created by Ahmad Qureshi on 17/11/22.
//
...
final class CommonValidation {
    static func isValidName(_ name: String) -> Bool {
        ...
        let allowedSet = alphabetSet.union(CharacterSet(charactersIn: " "))
        let enteredSet = CharacterSet(charactersIn: name)
        let predicate = NSPredicate(format: "SELF MATCHES %@", "^[a-zA-Z]+$")
        let idealName = predicate.evaluate(with: name)
        
        if !enteredSet.isSubset(of: allowedSet) && idealName == false {
            return false
        }
